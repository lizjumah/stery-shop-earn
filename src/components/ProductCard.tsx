...
export const ProductCard = ({ product, showDealBadge = true }: ProductCardProps) => {
  ...
  return (
    <>
      {/*
        Layout contract:
        - Link is the grid item. Grid stretches it to the row height by default
          (align-items: stretch). We make it a flex column so its child can use flex-1.
        - Card div uses flex-1 to fill the Link without h-full (avoids cyclic height).
        - Image wrapper: aspect-[3/2] + shrink-0 locks the image height regardless of
          card height. absolute inset-0 image prevents any overflow bleed.
        - Content wrapper: flex-1 grows into the remaining space after the image.
        - Price/button row: mt-auto pins it to the bottom of the content wrapper.
      */}
      <Link
        to={isEarnMode ? `/earn/product/${product.id}` : `/shop/product/${product.id}`}
        className="flex flex-col"
      >
        <div className="flex flex-col flex-1 bg-card rounded-lg overflow-hidden card-elevated animate-fade-in">

          {/* Image — fixed aspect ratio, never grows or shrinks */}
